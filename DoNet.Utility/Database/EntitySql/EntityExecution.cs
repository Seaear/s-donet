...
namespace DoNet.Utility.Database.EntitySql
{
    ...
    public static class EntityExecution
    {
        ...
        ///     插入实体
        /// </summary>
        public static void Insert<T>(this T entity, DbHelper db = null)
            where T : BaseEntity
        {
            if (db == null) db = DbFactory.CreateDatabase();
            using (var cmd = SqlCreator.CreateInsertCommand(db, entity))
            {
                var rowCount = db.ExecuteNonQuery(cmd);
        ...
        ///     插入实体，并返回标识列的值
        /// </summary>
        public static object InsertWithIdentity<T>(this T entity, DbHelper db = null)
            where T : BaseEntity
        {
            if (db == null) db = DbFactory.CreateDatabase();
            using (var cmd = SqlCreator.CreateInsertCommandWithIdentity(db, entity))
            {
                var retObj = db.ExecuteScalar(cmd);
                return retObj;
            }
        }
        ...
        ///     删除实体类
        /// </summary>
        public static int Delete<T>(Expression<Func<T, bool>> conditionExpression, DbHelper db = null)
            where T : BaseEntity
        {
            ...
            if (db == null) db = DbFactory.CreateDatabase();
            var whereEntity = new GenericWhereEntity<T>();
        ...
        ///     删除实体类
        /// </summary>
        public static int Delete<T>(GenericWhereEntity<T> whereEntity, DbHelper db = null)
            where T : BaseEntity
        {
            //DELETE时应该禁用别名
            ...
            if (db == null) db = DbFactory.CreateDatabase();
            using (var cmd = SqlCreator.CreatDeleteCommand(db, whereEntity))
            {
        ...
        ///     删除实体类
        /// </summary>
        public static int Delete<T>(this T entity, DbHelper db = null)
            where T : BaseEntity
        {
            //获取主键相关的字段
            ...
            if (db == null) db = DbFactory.CreateDatabase();
            using (var cmd = SqlCreator.CreatDeleteCommand(db, entity))
            {
                return db.ExecuteNonQuery(cmd);
            }
        }
        ...
        ///     更新实体类
        /// </summary>
        public static void Update<T>(this T entity, DbHelper db = null)
            where T : BaseEntity
        {
            //检索数据库相关的字段和属性
            var entityType = typeof (T);
            ...
            if (db == null) db = DbFactory.CreateDatabase();
            using (var cmd = SqlCreator.CreateUpdateCommand(db, entity))
            {
                var rowCount = db.ExecuteNonQuery(cmd);
        ...
        ///     更新实体类
        /// </summary>
        public static void Update<T>(this T entity, Expression<Func<T, bool>> conditionExpression, DbHelper db = null)
            where T : BaseEntity
        {
            if (conditionExpression == null)
        ...
        ///     更新实体类
        /// </summary>
        public static void Update<T>(this T entity, GenericWhereEntity<T> whereEntity, DbHelper db = null)
            where T : BaseEntity
        {
            if (db == null) db = DbFactory.CreateDatabase();
            using (var cmd = SqlCreator.CreateUpdateCommand(db, entity, whereEntity))
            {
        ...
        public static void SetMemberNull<T, TKey>(Expression<Func<T, bool>> conditionExpression,
            Expression<Func<T, TKey>> keySelector, DbHelper db = null)
            where T : BaseEntity
        {
            if (conditionExpression == null)
                throw new EntitySqlException("修改记录时，必须指定Where条件，否则将导致整个表的数据被修改！");
        ...
        ///     将指定的字段设置为Null
        /// </summary>
        public static void SetMembersNull<T>(Expression<Func<T, bool>> conditionExpression, DbHelper db = null,
        ...
        {
            var entityType = typeof (T);
            var m = keySelector.Body as MemberExpression;
            var memberName = m.Member.Name;

            if (db == null) db = DbFactory.CreateDatabase();
            using (var cmd = SqlCreator.CreateUpdateMemberToNullCommand(db, whereEntity, memberName))
            {
                db.ExecuteNonQuery(cmd);
            }
        }
        ...
        ///     将指定的字段设置为Null
        /// </summary>
        public static void SetMembersNull<T>(GenericWhereEntity<T> whereEntity, DbHelper db = null,
            params Expression<Func<T, object>>[] keySelectors)
            where T : BaseEntity
        {
            var entityType = typeof (T);
            ...
            //检索要UPDATE的成员列表
            var memberNames = new List<string>(keySelectors.Length);
            for (var i = 0; i < keySelectors.Length; i++)
            {
                //对于引用类型的成员访问,使用如下转换
                var m = keySelectors[i].Body as MemberExpression;
                if (m != null)
                {
            ...
            if (db == null) db = DbFactory.CreateDatabase();
            using (var cmd = SqlCreator.CreateUpdateMemberToNullCommand(db, whereEntity, memberNames))
            {
                db.ExecuteNonQuery(cmd);
            }
        }
        ...
        /// <returns></returns>
        public static T SelectOne<T>(Expression<Func<T, bool>> conditionExpression, DbHelper db = null)
        ...
        public static T SelectOne<T>(GenericWhereEntity<T> whereEntity, DbHelper db = null)
            where T : BaseEntity, new()
        {
            var selectSql = SqlCreator.CreateSelectSql(whereEntity, 1);
            var whereSql = SqlCreator.CreateWhereSql(whereEntity);
        ...
        public static List<T> SelectAll<T>(Expression<Func<T, bool>> conditionExpression = null, DbHelper db = null,
        ...
        /// <param name="maxRowCounts">读取的记录数量</param>
        /// <returns>实体类的列表</returns>
        public static List<T> SelectAll<T>(GenericWhereEntity<T> whereEntity, DbHelper db = null,
            params int[] maxRowCounts)
            where T : BaseEntity, new()
        {
            var rowCount = (maxRowCounts == null || maxRowCounts.Length < 1) ? 0 : maxRowCounts[0];
            var selectSql = SqlCreator.CreateSelectSql(whereEntity, rowCount);
            var whereSql = SqlCreator.CreateWhereSql(whereEntity);
        ...
        public static int Count<T>(Expression<Func<T, bool>> conditionExpression, DbHelper db = null)
            where T : BaseEntity
        {
            var whereEntity = new GenericWhereEntity<T>();
            if (conditionExpression != null)
        ...
        /// <param name="db">数据库连接</param>
        /// <returns>符合条件的实体的数量</returns>
        public static int Count<T>(GenericWhereEntity<T> whereEntity, DbHelper db = null)
            where T : BaseEntity
        {
            var whereSql = SqlCreator.CreateWhereSql(whereEntity);
